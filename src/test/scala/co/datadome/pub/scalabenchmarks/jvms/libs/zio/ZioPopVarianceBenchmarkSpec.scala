...
class ZioPopVarianceBenchmarkSpec extends TestSuite {
  val Tolerance = 0.000000001d

  private def withBench[A](size: Int)(f: ZioPopVarianceBenchmark => A): A = {
    val bench = new ZioPopVarianceBenchmark()
    bench.popSize = size
    bench.setup()
    f(bench)
  }
